...
class DepartmentsController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        $branch = new branch();
        $allBranchs = $branch->getAllBranchs();
    ...
    public function actionList_branch($id)
    {
        //count
        $countbranch = Branch::find()->where(['company_id'=>$id])->count();

        $branch = Branch::find()->where(['company_id'=>$id])->orderBy('id DESC')->all();
        if($countbranch > 0 ){
            foreach ($branch as $result) {
                echo "<option value='".$result->id."'>".$result->branch_name."</option>";
            }
        }else{
